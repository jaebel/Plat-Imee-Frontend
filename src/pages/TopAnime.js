...
const TopAnime = () => {
  ...
  // Fetch top anime from Jikan when the component mounts
  useEffect(() => {
    setLoading(true);
    setError('');
  ...
  // Add anime to user’s local list
  ...
  // View Details logic: check if local DB has it, if not, create minimal record
  const handleViewDetails = async (item) => {
    ...
    try {
      ...
      if (err.response && err.response.status === 404) {
        try {
          // Build the payload from Jikan's item
          const jikanType = mapJikanType(item.type);
          const minimalGenres = []; // or parse if you want real genre IDs
